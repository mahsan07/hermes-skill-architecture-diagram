<p align="center">
  <img src="assets/system-map.svg" alt="Detailed system map for Architecture Diagram" width="100%">
</p>
...
Creative requests often lose their intent between the initial brief, generation, export, and final quality check. **Architecture Diagram** turns that work into an explicit sequence with visible inputs, outputs, review points, and completion evidence.

## Why the repository has this name

The shared `hermes-skill-` prefix identifies this as a portable Hermes workflow package. `architecture-diagram` names the capability directly—architecture diagram—so the repository remains searchable and understandable outside the original AI-OS workspace. The public title is **Architecture Diagram**.
...
| What does it do? | Create dark, readable SVG architecture diagrams from system descriptions. |
...
![Architecture Diagram system map](assets/system-map.svg)

## Operation sequence

![Architecture Diagram actor and data sequence](assets/operation-sequence.svg)
...
![Illustrative output produced by Architecture Diagram](assets/example-output.svg)
...
> Using original or licensed sample material, create dark, readable SVG architecture diagrams from system descriptions. Return the result, the evidence used to verify it, and any limitations or actions that still require approval.
