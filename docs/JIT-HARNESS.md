...
This package expresses **Architecture Diagram** as a typed, task-adaptive agent harness. Its concrete capability remains: Create dark, readable SVG architecture diagrams from system descriptions. The harness contract makes memory, planning, action, capability selection, failure handling, and verification explicit.
...
| Module | Strategy | Responsibility in Architecture Diagram |
| --- | --- | --- |
| Memory | `stage_artifact_log` | Retain system brief plus decisions, observations, and verification evidence needed to resume safely. |
| Planning | `linear_gated_plan` | Transform component inventory into the ordered, bounded stages defined by this skill. |
...
| Action | `brief_generate_render_inspect_export` | Advance the workflow through relationship model while preserving stop conditions and user authority. |
